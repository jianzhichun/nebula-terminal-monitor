...
interval_ = max(int(sys.argv[2]), 5) if argv_len > 2 and sys.argv[1] else 5
range_ = max(int(sys.argv[3]), 12) if argv_len > 3 and sys.argv[1] else 12

def fetch():
    r = requests.get(f'{url}/stats?stats=slow_query_latency_us.avg.5,slow_query_latency_us.p99.5&format=json')
    for d in r.json():
        for k, v in d.items():
            data[k].append(v)
